...
void Conv_layer::sback(sfloat *dest, sfloat *gradient,  sfloat *last_input) {
    sfloat *holder;
    holder = new sfloat[filter_size*filter_size*colors];
    int r, c, n, i, j, k;
...
void strain(Conv_layer &conv, Avgpool_layer &avgpool, Softmax_layer &softmax, unsigned char *image, float *filters, 
    ...
    sfilters = new sfloat[filter_size*filter_size*num_filters*3];
    spool_out = new sfloat[30*30*num_filters];
    ...
    for (int i = 0; i < filter_size*filter_size*num_filters*3; i++) {
